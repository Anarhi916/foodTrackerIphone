...
struct NutrientData: Codable, Equatable {
    ...
    func macrosList() -> [(key: String, name: String, value: Double)] {
        [
            ("calories", L("Калории (ккал)"), calories),
            ("protein", L("Белки (г)"), protein),
            ("fat", L("Жиры (г)"), fat),
            ("carbs", L("Углеводы (г)"), carbs),
            ("fiber", L("Клетчатка (г)"), fiber)
        ]
    }

    func fatDetailsList() -> [(key: String, name: String, value: Double)] {
        [
            ("saturatedFat", L("Насыщенные жиры (г)"), saturatedFat),
            ("monounsaturatedFat", L("Мононенасыщенные жиры (г)"), monounsaturatedFat),
            ("polyunsaturatedFat", L("Полиненасыщенные жиры (г)"), polyunsaturatedFat),
            ("cholesterol", L("Холестерин (мг)"), cholesterol)
        ]
    }

    func vitaminsList() -> [(key: String, name: String, value: Double)] {
        [
            ("vitaminA", L("Витамин A (мкг)"), vitaminA),
            ("vitaminB1", L("Витамин B1 (мг)"), vitaminB1),
            ("vitaminB2", L("Витамин B2 (мг)"), vitaminB2),
            ("vitaminB3", L("Витамин B3 (мг)"), vitaminB3),
            ("vitaminB5", L("Витамин B5 (мг)"), vitaminB5),
            ("vitaminB6", L("Витамин B6 (мг)"), vitaminB6),
            ("vitaminB7", L("Витамин B7 (мкг)"), vitaminB7),
            ("vitaminB9", L("Витамин B9 (мкг)"), vitaminB9),
            ("vitaminB12", L("Витамин B12 (мкг)"), vitaminB12),
            ("vitaminC", L("Витамин C (мг)"), vitaminC),
            ("vitaminD", L("Витамин D (мкг)"), vitaminD),
            ("vitaminE", L("Витамин E (мг)"), vitaminE),
            ("vitaminK", L("Витамин K (мкг)"), vitaminK)
        ]
    }

    func mineralsList() -> [(key: String, name: String, value: Double)] {
        [
            ("calcium", L("Кальций (мг)"), calcium),
            ("iron", L("Железо (мг)"), iron),
            ("magnesium", L("Магний (мг)"), magnesium),
            ("phosphorus", L("Фосфор (мг)"), phosphorus),
            ("potassium", L("Калий (мг)"), potassium),
            ("sodium", L("Натрий (мг)"), sodium),
            ("zinc", L("Цинк (мг)"), zinc),
            ("copper", L("Медь (мг)"), copper),
            ("manganese", L("Марганец (мг)"), manganese),
            ("selenium", L("Селен (мкг)"), selenium),
            ("iodine", L("Йод (мкг)"), iodine)
        ]
    }
