...
final class AuthManager: NSObject, ObservableObject {
        ...
        let session = ASWebAuthenticationSession(url: authURL, callbackURLScheme: scheme) { [weak self] callback, error in
            ...
            Task { @MainActor in
                guard let callback,
                    ...
                    if error != nil { self.errorMessage = L("Вход через Google отменён") }
                    return
                }
    ...
    private func exchangeGoogleCode(code: String, codeVerifier: String, redirectUri: String) async {
        ...
        do {
            ...
            isAuthenticating = false
            errorMessage = L("Не удалось войти через Google")
        }
    }
...
extension AuthManager: ASAuthorizationControllerDelegate {
    func authorizationController(controller: ASAuthorizationController,
        ...
        guard let cred = authorization.credential as? ASAuthorizationAppleIDCredential,
            ...
            isBusy = false
            isAuthenticating = false
            errorMessage = L("Apple не вернул токен")
            return
        }
        Task { @MainActor in
            defer { isBusy = false }
            do {
                ...
                isAuthenticating = false
                errorMessage = L("Не удалось войти через Apple")
            }
        }
    }
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithError error: Error) {
        ...
        if (error as? ASAuthorizationError)?.code != .canceled {
            errorMessage = L("Вход через Apple не удался")
        }
    }
}
